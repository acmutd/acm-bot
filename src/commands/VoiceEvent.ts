        ...
        if (args[0] == 'start') {
            ...
            if (client.indicators.hasKey('voiceEvent', voiceChannel.id)) {
                return client.response.emit(
                    msg.channel,
                    `<@${
                        client.indicators.getValue('voiceEvent', voiceChannel.id).moderatorId
            ...
            if (isNaN(points) || points < -100 || points > 100) {
                return client.response.emit(
                    msg.channel,
                    `\`${args[2]}\` is not a valid number of points between -100 and 100 (inclusive).`,
                    'invalid'
                );
            }
            ...
            return client.response.emit(msg.channel, 'Voice event has started!', 'success');
        } else if (args[0] == 'stop') {
            let activityId: string;
            let points: number;
            ...
            if (voiceEvent) {
                ...
                return msg.reply(
                    `Voice event has stopped!\nAwarded **${voiceEvent.points}** points to **${
                        success.length
                    }** users for completing **${voiceEvent.activityId}**:\n${success.join(
                        ' '
                    )}\n` +
                        (failure.length
