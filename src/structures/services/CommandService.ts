...
export default class CommandService {
    ...
    canInvoke(msg: Message, command: Command): string | MessageEmbed | void {
        ...
        if(command.userPermissions && msg.member && !msg.member.permissions.has(command.userPermissions))
            return this.client.response.build('You do not have the appropriate permissions to use this command.', 'invalid');

        if(this.client.config.disabledCommands && this.client.config.disabledCommands.includes(command.name))
