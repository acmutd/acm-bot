...
export default class CommandManager extends Manager {
    ...
    public cantInvoke(msg: Message, command: Command): string | MessageEmbed | void {
        const dm = msg.channel instanceof DMChannel
        if (dm && !command.dmWorks) return this.bot.response.build('DM functionality is not enabled for this command...', 'invalid')
        if (command.userPermissions && msg.member && !msg.member.permissions.has(command.userPermissions)) return this.bot.response.build('Invalid permission level...', 'invalid')
        if (command.requiredRoles && msg.member && !msg.member?.roles.cache.some(role => command.requiredRoles!.findIndex(rr => rr == role.id) != -1)) {
            const role = msg.guild?.roles.cache.get(command.requiredRoles[0])
            return this.bot.response.build(`Missing role '${role?.toString()}'...`, 'invalid')
        }
        if (this.bot.config.disabledCommands && this.bot.config.disabledCommands.includes(command.name)) return this.bot.response.build('Command disabled...', 'invalid')
    }
    public handle(msg: Message): void {
        ...
        if (!cmd) return this.bot.response.emit(msg.channel, `Invalid command...`, 'invalid')
        if (this.bot.managers.indicator.hasUser('usingCommand', msg.author)) return this.bot.response.emit(msg.channel, 'You are already using a command. Please complete that action before beginning another...', 'invalid')
