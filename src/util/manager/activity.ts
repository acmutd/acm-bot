...
export default class ActivityManager extends Manager {
  ...
  async handleMessage(msg: Message): Promise<void> {
    ...
    if ((indicators.hasKey('textActivity', msg.author.id) && msg.createdTimestamp > indicators.getValue('textActivity', msg.author.id)! + cooldown * 1000) || !indicators.hasKey('textActivity', msg.author.id)) {
      indicators.setKeyValue('textActivity', msg.author.id, msg.createdTimestamp)
      let { success, failure } = await this.bot.managers.points.awardPoints(1, 'Discord', new Set<string>([msg.author.id]))
      if (success.length === 0) { }
      else {
