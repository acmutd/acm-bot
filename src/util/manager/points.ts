import { MessageOptions, DMChannel, MessageEmbed, MessageReaction, User, VoiceChannel, Collection, TextChannel } from 'discord.js'
...
export default class PointsManager extends Manager {
  ...
  async handleReactionAdd(reaction: MessageReaction, user: User) {
    ...
    if (user.id === this.bot.user?.id || msg.channel.id !== this.privateChannelId || msg.author.id !== this.bot.user?.id || !reaction.users.cache.has(this.bot.user?.id) || reaction.emoji.name !== "✅" || msg.embeds.length !== 1 || !msg.embeds[0].title || !msg.embeds[0].title!.startsWith("Response for") || !msg.embeds[0].description || !re.test(msg.embeds[0].description)) return
  ...
  async handlePointsTypeform(typeformData: any) {
    ...
    const confirmationChannel = (await this.bot.channels.fetch(this.privateChannelId)) as TextChannel
    const resolvedSnowflakes: string[] = await this.emailsToSnowflakes(new Set<string>([answers[0].email]))

    if (resolvedSnowflakes.length === 0) return this.bot.response.emit(confirmationChannel, `\`${answers[1].text}\` submitted \`${title}\` with an unknown email: \`${answers[0].email}\``,
      'error')
    ...
    if (answers[4].type === 'text') embed.description += "\n*" + answers[4].text + "*"
  ...
  async handleRegistrationTypeform(typeformData: any) {
    const notifChannel = (await this.bot.channels.fetch(this.privateChannelId)) as TextChannel
    ...
    if (mentorSnowflake && menteeSnowflake) {
      ...
      await this.bot.managers.firestore.firestore?.collection("points_system").doc('pairs').set({
  ...
  async registerUser(data: UserPointsData, notify: boolean = true) {
    ...
    const member = await this.bot.managers.resolve.resolveGuildMember(data.tag, guild, new Set<string>(['tag']), true)
    if (!member) {
    ...
    await this.bot.managers.firestore.firestore?.collection("points_system/users/profiles").doc(data.snowflake).set(data, { merge: true })

    await this.bot.managers.firestore.firestore?.collection("points_system").doc('email_to_discord').set({ [data.email]: data.snowflake }, { merge: true })
    if (notify) await member.send(new MessageEmbed({
      color: '#ec7621',
    ...
    })).catch((e: any) => notifChannel.send(`DMs are off for ${data.tag} (${data.full_name})`))
    return data.snowflake
  }
  async handleGenericTypeform(typeformData: any) {
    const points: number = typeformData.form_response.calculated.score
    const answers: any = typeformData.form_response.answers
    const errChannel = (await this.bot.channels.fetch(this.privateChannelId)) as TextChannel
    const title: string = typeformData.form_response.definition.title

    const resolvedSnowflakes: string[] = await this.emailsToSnowflakes(new Set<string>([answers[0].email]))
    if (resolvedSnowflakes.length === 0) return this.bot.response.emit(errChannel, `\`${answers[1].text}\` submitted \`${title}\` with an unknown email: \`${answers[0].email}\``, 'error')

    await this.awardPoints(points, title, new Set<string>([resolvedSnowflakes[0]]))
  }

  startReactionEvent(channelId: string, activityId: string, reactionId: string, moderatorId: string, points: number) {
    if (this.bot.managers.indicator.hasKey('reactionEvent', channelId)) return false
  ...
  startVoiceEvent(voiceChannel: VoiceChannel, activityId: string, moderatorId: string, points: number) {
    ...
    if (this.bot.managers.indicator.hasKey('voiceEvent', voiceChannel.id)) return false
  ...
  async awardPoints(points: number, activity: string, awardees: Set<string>) {
    ...
    for (const snowflake of awardees.values()) {
      const docRef = this.bot.managers.firestore.firestore?.collection("points_system/users/profiles").doc(snowflake)
      ...
      else userData.activities[activity] = (userData.activities[activity] || 0) + points
      userData.points = (userData.points || 0) + points
      await docRef?.set(userData)
      success.push(`<@${snowflake}>`)
    }
    if (activity !== 'Discord') {
      const logChannel = await this.bot.channels.fetch(this.publicChannelId) as TextChannel
      if (success.length > 60) logChannel.send(`Awarded ${points} to ${success.length} users for ${activity}...`, { "allowedMentions": { "users": [] } })
      else if (success.length !== 0) logChannel.send(`Awarded ${points} points to ${success.join(', ')} for ${activity}...`, { "allowedMentions": { "users": [] } })
    }
  ...
  async getUser(snowflake: string) {
    ...
    await this.bot.managers.firestore.firestore?.collection("points_system/users/profiles").doc(snowflake).get().then(async (doc) => {
      exists = doc.exists
  ...
  async emailsToSnowflakes(emails: Set<string>): Promise<string[]> {
    ...
    await this.bot.managers.firestore.firestore?.collection("points_system").doc("email_to_discord").get().then(async (doc) => {
  ...
  async getLeaderboard(type: 'mentee' | 'mentor' | 'both' = 'both', limit = 0) {
    ...
    const individualData: Map<string, UserPointsData> = new Map<string, UserPointsData>()
    const snapshot = await this.bot.managers.firestore.firestore?.collection("points_system/users/profiles").get()
    snapshot?.forEach((doc) => {
      individualData.set(doc.id, doc.data() as UserPointsData)
    })
    const pairs = await (await this.bot.managers.firestore.firestore?.collection("points_system").doc("pairs").get()!).data()
    switch (type) {
      case 'mentee':
