...
export default class LoggerUtil {
    ...
    private write(level: LogLevel, severity: LogSeverity, ...message: LogMessage): void {
        let lvlText!: string
        switch (level) {
            default:
            case LogLevel.INFO:
        ...
        appendFileSync(`${this.logPath}${sep}acmbot.log`, `${this.getDate()} ${this.strip(lvlText)} -> ${this.strip(msg)}\n`)
