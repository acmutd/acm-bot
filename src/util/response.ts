...
export default class ResponseUtil {
    ...
    private embed(msg: string, emojiSet: { simple: string, embed: string, color: ColorResolvable }): MessageEmbed {
        return new MessageEmbed().setDescription(`${emojiSet.embed} | **${msg}**`).setColor(emojiSet.color)
    }
    public build(message: string, type?: ResponseType, format?: ResponseFormat): string | MessageEmbed {
        type = type || 'normal'
    ...
    public emit(channel: TextChannel | DMChannel | NewsChannel, message: string, type?: ResponseType, format?: ResponseFormat): void {
        const response = this.build(message, type, format)
        typeof response == 'string' ? channel.send(response) : channel.send({ embed: response })
    }
    public emitBuild(channel: TextChannel | DMChannel | NewsChannel, response: string | MessageEmbed): void {
        typeof response == 'string' ? channel.send(response) : channel.send({ embed: response })
    }
}