...
export abstract class WizardNode {
    ...
    private implementDefaults(overwrites: MessageEmbedOptions): MessageEmbedOptions {
        ...
        if (this.wizard.defaults) {
            ...
            messageEmbedOptionKeys.forEach(key => {
                if (this.overwrites[key] && details[key]) details[key] = this.overwrites[key] as any
            })
        }
        if (this.wizard.additions) {
            const additions = this.wizard.additions
            messageEmbedOptionKeys.forEach(key => {
                if (typeof additions[key] === 'string' && typeof details[key] === 'string') details[key] = ((additions[key] as string) + details[key]) as any
            })
        }
        return details
    }

    public abstract preSendCB(details: MessageEmbedOptions): Promise<MessageEmbedOptions | void>;
    ...
    public async emit(): Promise<WizardNodeResponse> {
        ...
        if (!details.footer) {
            details.footer = {
                text: `${this.options.loopedCB ? "Enter '" + this.wizard.configs.commands.doneLoop + "' when you're finished. " : ''}Enter '${this.wizard.configs.commands.quit}' to end the wizard...`
            }
        }
        try {
        ...
        if (this.options.loopedCB) {
            ...
            do {
                do {
                    if (failed) {
                        details.color = 'RED'
                        if (this.options.invalidMessage) this.wizard.message.channel.send(this.options.invalidMessage)
                    }
                    ...
                    if (this.options.skipValue) {
                        if (response.first()?.content.toLowerCase() === this.wizard.configs.commands.skip) {
                            wizardNode.delete()
                            return { status: WizardNodeResponseStatus.COMPLETE, item: this.options.skipValue }
                        }
                    }
                    if (response.first()?.content.toLowerCase() === this.wizard.configs.commands.doneLoop) {
...
export class CustomWizardNode extends WizardNode {
    ...
    constructor(wizard: Wizard, overwrites: MessageEmbedOptions, callback: (arg0: Message) => any, options?: WizardNodeOptions) {
        super(wizard, overwrites, options)
        this.callback = callback
    }
    public async preSendCB(details: MessageEmbedOptions): Promise<MessageEmbedOptions | void> { }
...
export class EmojiWizardNode extends WizardNode {
    ...
    public async validationCB(response: Message): Promise<any> {
        ...
        if (matches !== null && matches.length > 0) return matches
    }
}
...
export class UserMentionWizardNode extends WizardNode {
    ...
    public async validationCB(response: Message): Promise<any> {
        if (response.mentions.users.array().length > 0) return response.mentions.users.first()
    }
}
export class ChannelMentionWizardNode extends WizardNode {
    ...
    public async validationCB(response: Message): Promise<any> {
        if (response.mentions.channels.array().length > 0) return response.mentions.channels.first()
    }
}
export class RoleMentionWizardNode extends WizardNode {
    ...
    public async validationCB(response: Message): Promise<any> {
        if (response.mentions.roles.array().length > 0) return response.mentions.roles.first()
    }
}
export class OptionsWizardNode extends WizardNode {
    ...
    constructor(wizard: Wizard, overwrites: MessageEmbedOptions, choices: string[], strict?: boolean, options?: WizardNodeOptions) {
        super(wizard, overwrites, options)
        this.strict = strict ?? true
        this.choices = choices
        this.numList = []
    }
    public async preSendCB(details: MessageEmbedOptions): Promise<MessageEmbedOptions> {
        let optionList = '\n'
    ...
    public async validationCB(response: Message): Promise<{ value: number | string; isOption: boolean } | undefined> {
...
export class ConfirmationWizardNode extends WizardNode {
    ...
    public async validationCB(response: Message): Promise<boolean | void> {
        if (typeof response.content == 'string' && response.content.toLowerCase() == 'confirm') return true
    }
}
