...
export default class VCEventCommand extends Command {
  ...
  public async exec({ msg, bot, args }: CommandContext) {
    ...
    const action = args[0]
    if (args.length > 1) {
      const chan = /^\d{17,19}$/.test(args[1]) ? await bot.channels.fetch(args[1]) : undefined
      if (!chan || chan.type !== 'voice') return bot.response.emit(msg.channel, 'Could not resolve that ID into a voice channel...', 'invalid')
      vc = chan as VoiceChannel
    } else {
      vc = msg.member?.voice.channel
      if (!vc && action !== 'list') return bot.response.emit(msg.channel, 'Please join a voice channel...')
    }
    let data
    switch (action) {
      case 'start':
      case 'begin':
        if (bot.managers.activity.startVoiceEvent(vc!)) return bot.response.emit(msg.channel, `VC Event started for ${vc}...`, 'success')
        else return bot.response.emit(msg.channel, `VC Event already running ${vc}...`, 'error')
      case 'stop':
      case 'end':
        data = bot.managers.activity.stopVoiceEvent(vc!)
        if (!data) return bot.response.emit(msg.channel, `No running VC Event in ${vc}...`, 'error')
