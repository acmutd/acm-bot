...
export default class ShoutoutCommand extends Command {
    ...
    public exec({ msg, bot, args }: CommandContext): void {
        if (!/^<@!?[\d]{17,18}>/.test(args[0])) return bot.response.emit(msg.channel, 'No user mentions...', 'invalid')
        ...
        if (receivers.length > 0) title += `${receivers.map((m) => m.nickname ?? m.user.username).join(', ')}${receivers.length > 0 ? ', and ' : ''}${last?.nickname ?? last?.user.username}`
        ...
        const channel = msg.guild?.channels.resolve(settings.channels.shoutout) as TextChannel
        channel.send(embed)
        channel.send(msg.content.match(reg)![0].replace(settings.prefix + 'shoutout', ''))
