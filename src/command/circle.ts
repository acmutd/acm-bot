...
import Wizard, { TextWizardNode, UserMentionWizardNode, EmojiWizardNode, GraphicWizardNode, ColorWizardNode } from '../util/wizard'
    ...
    public async exec({ msg, bot, args }: CommandContext): Promise<void> {
        switch (args[0]) {
            ...
            case 'repost':
                ...
                break
            default:
                msg.channel.send(`User '${settings.prefix}circle help' to show a list of commands...`)
                break
        }
    }
}
async function addCircle(bot: Bot, msg: Message, args: string[]) {
    const wizard = new Wizard(msg, undefined, { title: '__**Circle Creation**__ ' })
    wizard.addNodes([
    ...
    const circleCategory = msg.guild!.channels.cache.get(settings.channels.circlesCategory) as CategoryChannel
    const desc = `🎗️: ${circleRole.name}`
    const circleChannel = await msg.guild!.channels.create(`${circle.emoji} ${circle.name}`, {
        type: 'text',
        topic: desc,
