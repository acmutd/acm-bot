...
export default class VCSnapshotCommand extends Command {
    ...
    public async exec({ msg, bot, args }: CommandContext) {
        ...
        if (args.length > 0) {
            const chan = /^\d{17,19}$/.test(args[0]) ? await bot.channels.fetch(args[0]) : undefined
            if (!chan || chan.type !== 'voice') return bot.response.emit(msg.channel, `Could not resolve the given ID into a valid voice channel...`, 'invalid')
        ...
        ).catch(() => msg.channel.send(`**VC Snapshot request failed for ${msg.author}**`, { allowedMentions: { users: [] } }))
    }
}
