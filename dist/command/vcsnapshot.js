        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (args.length > 0) {
                const chan = /^\d{17,19}$/.test(args[0])
                    ? yield bot.channels.fetch(args[0])
                    : undefined;
            ...
            msg.channel
                .send(`**VC Snapshot for \`${voiceChannel.name}\` requested by ${msg.author}**\n` +
                `Members (${attendees.length}): ${attendees.join(" ")}\n` +
                `Copyable: \` ${attendees.join("` `")}\``, { allowedMentions: { users: [] } })
                .catch(() => msg.channel.send(`**VC Snapshot request failed for ${msg.author}**`, {
