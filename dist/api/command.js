...
class Command {
    ...
    infoEmbed() {
        const embed = new discord_js_1.MessageEmbed().setTitle(`Command ${this.name}`).setDescription(`**${this.description}**`).addField('Usage', this.usage.length > 0 ? this.usage.join(', ') : 'No usage cases available', true).addField('Tags', this.tags.length > 1 ? this.tags.join(', ') : 'No tags', true).addField('Works in DMs?', this.dmWorks ? 'Yes' : 'No', true).addField('Cooldown', `${this.cooldown} seconds`, true);
        return embed;
    }
