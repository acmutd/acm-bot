...
class CommandManager extends manager_1.default {
    ...
    cantInvoke(msg, command) {
        ...
        if (command.userPermissions && msg.member && !msg.member.permissions.has(command.userPermissions))
            return this.bot.response.build('Invalid permission level...', 'invalid');
        if (command.requiredRoles && msg.member && !((_a = msg.member) === null || _a === void 0 ? void 0 : _a.roles.cache.some(role => command.requiredRoles.findIndex(rr => rr == role.id) != -1))) {
        ...
        if (this.bot.config.disabledCommands && this.bot.config.disabledCommands.includes(command.name))
    ...
    handle(msg) {
        ...
        const args = shlex_1.default.split(msg.content.slice(settings_1.settings.prefix.length).trim()).slice(1);
